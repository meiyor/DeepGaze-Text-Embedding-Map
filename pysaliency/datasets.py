...
class Fixations(object):
    ...
    def read_hdf5(cls, source):
        ...
        #print(fixations.x_hist,fixations.y_hist,'heyy')
        return fixations
...
class FixationTrains(Fixations):
    ...
    """
    #def __init__(self, train_xs, train_ys, train_ts, x_hist, y_hist, t_hist, train_ns, train_subjects, attributes=None):
    ...
    @classmethod
    def FixationsWithoutHistory_Train(cls, x, y, t, n, subjects):
        x_hist = np.empty((len(x), 1))*np.nan
        y_hist = np.empty((len(x), 1))*np.nan
        t_hist = np.empty((len(x), 1))*np.nan
        #print(cls,x,y,t)
...
#        return type(self)(train_xs, train_ys, train_ts, train_ns, train_subjects)
    ...
    def to_hdf5(self, target):
        ...
        #print(self.__attributes__,self.x_hist,self.y_hist)
        for attribute in ['train_xs', 'train_ys', 'train_ts', 'train_ns', 'x_hist', 'y_hist', 't_hist', 'train_ns', 'train_subjects'] +self.__attributes__:
    ...
    def read_hdf5(cls, source):
        ...
        data_type = decode_string(source.attrs['type'])
        data_version = decode_string(source.attrs['version'])
        #print(data_type,data_version,source,'jii')
        ...
        #data = {key: source[key][...] for key in ['train_xs', 'train_ys', 'train_ts', 'x_hist', 'y_hist', 't_hist', 'train_ns']}

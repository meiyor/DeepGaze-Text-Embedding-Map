...
class ImageDataset_TEM(Dataset):
    ...
    def __getitem__(self, key):
        if not self.cached or key not in self._cache:
            ...
            # shuffle the key indices for the key index on the TEM
            # print(key,'key')

            key_shuffled = np.random.randint(0, len(self.TEM.stimuli), 1)
            if key_shuffled[0] == key:
                key_shuffled[0] = 0

            TEM_image = np.array(self.TEM.stimuli[key_shuffled[0]])
            ...
            if not(self.centerbias_TEM is None):

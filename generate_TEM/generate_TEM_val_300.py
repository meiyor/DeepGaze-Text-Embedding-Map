...
output_dir='TEM_output_all_val_output'
...
ind=0
cc=0
print(len(onlyfiles_val))
for file_name in onlyfiles_val:
    ...
    for k in range(0,len(annotation_object)):
       ...
       if file_annotated[0] == file_name_def:
           ...
           img_sal = np.zeros((img.shape[0],img.shape[1],303))
           ...
           for i in range(0,len(segment_burst)):
               ...
               img_sal[mask]=np.concatenate((np.squeeze(np.reshape(vector_data[ind_val],(-1,300))-np.reshape(vector_data[pos_n],(-1,300))),[val_dist[0][0],val_dist_1[0][0],val_dist_2[0][0]]))
           img_sal[boundaries_img] = np.zeros((303))
           break
    if file_annotated[0] == file_name_def: 
         print('COCO_val2014_'+file_annotated[0]+'.tif',cc)
         cc=cc+1
         imwrite(os.path.join(output_dir,'COCO_val2014_'+file_annotated[0]+'.tif'),img_sal[::2,::2,:],compress=6)

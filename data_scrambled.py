...
class ImageDataset_TEM(Dataset):
    ...
    def __getitem__(self, key):
        if not self.cached or key not in self._cache:
            ...
            TEM_image_s = TEM_image
            # print(TEM_image.shape,'shape0')
            s = imageshuffle.Rand(1234)  # ,[10,10])
            TEM_image_o = s.enc(TEM_image)
            dec = s.dec(TEM_image_o)
            # print(TEM_image_o.shape,'shape1')
            # print(np.max(TEM_image_o-TEM_image),np.min(TEM_image_o-TEM_image),'max_min')
            TEM_image = TEM_image_o
